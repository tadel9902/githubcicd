...
<link
    ...
    <link rel="stylesheet" href="<c:url value = '/include/css/admin/author_admin.css' />" />
...
<body>
	<div id="app doan">
		<div class="background_black" id="background_black">
			...
			<form action = "<c:url value = '/admin/author/delete.htm' />" method="post" class="confirm_container remove_class bg-white">
				<input type="hidden" id = "input_id_delete" name="idAuthor" value="1">
				<div class="confirm_header text-white bg-primary"
					style="padding: 10px; font-weight: 700">
					<i class="fas fa-check-circle" style="color: #47f764"></i> Thông báo
				</div>
				...
				<div class="confirm_buttons">
					<button class="btn btn-success me-1 mb-2 btn_xacnhan"
						style="margin: 0px 10px" type="submit">Xác Nhận
					</button>
					<button class="btn btn-danger me-1 mb-2 btn_huy_cancel"
						style="margin: 0px 10px" type="button">Hủy
					</button>
				</div>
			</form>
		</div>
									...
									<div class="card">
										<div class="card-body">
											<div
												...
												<h4 class="card-title" style="margin-bottom: 0 !important">
													Danh sách tác giả</h4>
												<div style="display: flex;">
													<button id="info" class="btn btn-success btn_add">
														<i class="fas fa-plus"></i> <span>Thêm Tác Giả</span>
													</button>
													<div class="form_add display_none"
														style="margin-left: 20px;">
														<form action="<c:url value = '/admin/author/add.htm' />" method="post" style="display: flex;">
															<input class="form-control"
																name="authorName" type="text" value="" id = "input_add_author"
																placeholder="VD: Nguyễn Đức Ngọc">
															<button id = "btn_add_submit"  style="margin-left: 5px;" type="submit"
																class="btn btn-success btn_add_submit">Thêm</button>
														</form>
													</div>
											...
											<div class="table-responsive pt-3">
												<table class="table table-bordered text-center">
													<thead>
														<tr>
															<th><a href="<c:url value = '/admin/author.htm?page=${currentpage}&order=id&dir=${orderLink}' />">ID</a></th>
															<th style="width: 65%;"><a href="<c:url value = '/admin/author.htm?page=${currentpage}&order=name&dir=${orderLink}' />">Tên tác giả</a></th>
															<th>Thay Đổi</th>
														</tr>
													</thead>
													<tbody id = "author_data">
													
													<c:forEach var = "item" items = "${authors}">
														<tr class="table-white">
															<td>${item.id_author}</td>
															<td><span class="ten_hien_thi">${item.author_name}</span>
																<div class="form_chinh_sua display_none">
																	<form action="<c:url value = '/admin/author/edit.htm' />" method="post" style="display: flex;">
																		<input type="hidden" name="idAuthor" value="${item.id_author}">
																		<input class="form-control class_input_edit"
																			name="authorName" type="text" value="${item.author_name}" />
		...
		<form action="<c:url value = '/admin/author.htm' />" id = "form_page">
		  ...
		  btn_edit.forEach(element => {
			...
			btn_delete.forEach(element => {
			    element.addEventListener("click", () => {
			        ...
			        var ten_tacgia = element.children[0].getAttribute("tentacgia");
			        $("#input_id_delete").val(id_delete);
			        confirm_content.innerHTML = "Bạn chắc chắn muốn xóa tác giả có ID = "+id_delete+ " <p><b>"+ten_tacgia+"</b></p>";
				...
				btn_huy_cancel.addEventListener("click", () => {
					...
					var input_add_author = document.querySelector("#input_add_author");
					input_add_author.addEventListener("change", (e) => {
						input_add_author.setAttribute("value", e.target.value);
					})
			    	...
			    	if(result != ""){
			    		if(result == '-1'){ // < 2 ky tu
				    		// Toast thất bại
					  		createToastHTML("Tên tác giả phải từ 2 ký tự trở lên", "ERROR", "#DC3545", "fas fa-exclamation-triangle");
				    	}else if(result == '0'){ // == 0 sai order_old
				    		// Toast thất bại
					  		createToastHTML("Tên tác giả đã tồn tại trên hệ thống.", "ERROR", "#DC3545", "fas fa-exclamation-triangle");
				    	}else if(result == '1'){
				    		// Toast thành công
					  		createToastHTML("Thêm tác giả thành công.", "SUCCESS", "#47f764", "fas fa-check-circle");
				    	}else if(result == '2'){
				    		// Toast thất bại
					  		createToastHTML("Thêm tác giả thất bại.", "ERROR", "#DC3545", "fas fa-exclamation-triangle");
				    	}
			    	}
			    	...
			    	if(result != ""){
			    		if(result == '-1'){ // < 2 ky tu
				    		...
				    		// Toast thành công
					  		createToastHTML("Chỉnh sửa tên tác giả thành công.", "SUCCESS", "#47f764", "fas fa-check-circle");
				    	}else if(result == '2'){
				    		// Toast thất bại
					  		createToastHTML("Chỉnh sửa tên tác giả thất bại.", "ERROR", "#DC3545", "fas fa-exclamation-triangle");
				    	}
			    	}
			    	...
			    	if(result != ""){
				    	if(result == '0'){ // == 0 sai order_old
				    		// Toast thất bại
					  		createToastHTML("Tác giả đã có sách trên hệ thống.", "ERROR", "#DC3545", "fas fa-exclamation-triangle");
				    	}else if(result == '1'){
				    		// Toast thành công
					  		createToastHTML("Xóa tác giả thành công.", "SUCCESS", "#47f764", "fas fa-check-circle");
				    	}else if(result == '2'){
				    		// Toast thất bại
					  		createToastHTML("Xóa tác giả thất bại.", "ERROR", "#DC3545", "fas fa-exclamation-triangle");
				    	}
			    	}
	...
	<script type="text/javascript" src="author_admin.js"></script>
</body>
</html>
